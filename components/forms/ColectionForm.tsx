...
interface Props {
  type?: string;
  colectionDetails?: string;
}

const ColectionForm = ({ type, colectionDetails }: Props) => {
  ...
  const parsedColectionDetails =
    colectionDetails && JSON.parse(colectionDetails || "");

  const form = useForm<z.infer<typeof SliderSchema>>({
    resolver: zodResolver(SliderSchema),
    defaultValues: {
      title: parsedColectionDetails?.title || "",
      subtitle: parsedColectionDetails?.subtitle || "",
      image: parsedColectionDetails?.image || "",
      link: parsedColectionDetails?.link || "",
    },
  });
  ...
  async function onSubmit(values: z.infer<typeof SliderSchema>) {
    ...
    try {
      if (type === "Edit" && !updateImage) {
        await editColection({
          colectionId: parsedColectionDetails._id,
          title: values.title,
  ...
  return (
    <>
      ...
      >
        {isHidden ? "Create Colection" : "Hide form"}
      </button>
          ...
          >
            ...
            >
              {isSubmiting ? (
                ...
                <>{type === "Edit" ? "Edit colection" : "Create Colection"} </>
              )}
            </Button>
